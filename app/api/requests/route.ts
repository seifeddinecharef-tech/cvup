...
export async function GET() {
  return NextResponse.json(
    {
      build: "runtime-fix-v3",
      projectRef: "zzkywyyubveutajwmwjv",
    },
    {
      headers: {
        "X-CVUP-BUILD": "runtime-fix-v3",
      },
    }
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!payload.full_name || !payload.email || !payload.cv_type || !payload.form_language) {
      return NextResponse.json({ success: false, error: "Missing required fields." }, { status: 400 });
    }

    const requestCode = generateRequestCode();
    console.log(JSON.stringify({ stage: "generated_request_code", request_code: requestCode }));
    ...
    const insertPayload = {
      ...
      has_current_cv: Boolean(payload.has_current_cv),
      ...
      has_certifications: Boolean(payload.has_certifications),
      ...
      recruitment_consent: Boolean(payload.recruitment_consent),
      final_consent: Boolean(payload.final_consent),
      admin_notes: null,
    ...
    console.log(JSON.stringify({ stage: "insert_success", inserted_request_code: data.request_code }));
    return NextResponse.json(
      {
        success: true,
        id: data.id,
