...
export function AdminPaymentControls({ requestCode, payment }: { requestCode: string; payment: Payment }) {
  ...
  return (
    <section className="admin-panel">
      <div className="admin-panel__heading"><div><p className="admin-eyebrow">Manual payment</p><h2>Payment tracking</h2></div><span className={`payment-badge payment-badge--${form.payment_status.toLowerCase()}`}>{form.payment_status}</span></div>
      <div className="admin-form-grid">
        <label>Status<select value={form.payment_status} onChange={(event) => setForm({ ...form, payment_status: event.target.value })}><option value="PENDING">PENDING</option><option value="PAID">PAID</option><option value="NOT_REQUIRED">NOT_REQUIRED</option><option value="REFUNDED">REFUNDED</option></select></label>
        <label>Method<input value={form.payment_method} onChange={(event) => setForm({ ...form, payment_method: event.target.value })} placeholder="Sofizpay" /></label>
      ...
      <div className="admin-panel__actions"><button type="button" className="admin-button admin-button--primary" onClick={save} disabled={saving}>{saving ? "Saving..." : "Save payment status"}</button>{message ? <span className="admin-message">{message}</span> : null}</div>
    </section>
  );
}